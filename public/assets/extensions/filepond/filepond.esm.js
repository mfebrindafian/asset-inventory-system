...
const createRoute =
     (routes, fn) =>
     ({ root, props, actions = [], timestamp, shouldOptimize }) => {
          actions.filter((action) => routes[action.type]).forEach((action) => routes[action.type]({ root, props, action: action.data, timestamp, shouldOptimize }));
          if (fn) {
...
const defaultOptions = {
     ...
     labelIdle: ['Seret & Jatuhkan file Anda atau  <span class="filepond--label-action">Jelajahi</span>', Type.STRING],
...
const getDateString = (date = new Date()) =>
     `${date.getFullYear()}-${leftPad(date.getMonth() + 1, '00')}-${leftPad(date.getDate(), '00')}_${leftPad(date.getHours(), '00')}-${leftPad(date.getMinutes(), '00')}-${leftPad(date.getSeconds(), '00')}`;
     ...
     ADD_ITEM: ({ source, index = -1, interactionMethod, success = () => {}, failure = () => {}, options = {} }) => {
          ...
          item.load(
               ...
               (file, success, error) => {
                    ...
                    applyFilterChain('LOAD_FILE', file, { query }).then(success).catch(error);
               }
          );
     },
     ...
     const processFiles = (...args) => {
          ...
          if (!queries.length) {
               const files = getFiles().filter(
                    (item) =>
                         !(item.status === ItemStatus.IDLE && item.origin === FileOrigin.LOCAL) && item.status !== ItemStatus.PROCESSING && item.status !== ItemStatus.PROCESSING_COMPLETE && item.status !== ItemStatus.PROCESSING_REVERT_ERROR
               );
